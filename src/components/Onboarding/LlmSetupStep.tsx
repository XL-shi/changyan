...
export function LlmSetupStep() {
  ...
  return (
    <div className="space-y-5">
      <Field label={t('onboarding.aiPolishService')}>
        <select
      ...
      <Field label={t('onboarding.apiKey')}>
        ...
        <TestStatusHint status={llmTestStatus} t={t} />
      </Field>
      ...
      <Field label={t('onboarding.model')}>
        <div className="flex gap-2">
          ...
          <button
            ...
            title={t('onboarding.fetchModels')}
          >
            <RefreshCw size={14} className={fetchingModels ? 'animate-spin' : ''} />
          </button>
        </div>
        {models.length > 0 && (
          <p className="text-[11px] text-text-tertiary mt-1">{t('onboarding.modelsAvailable', { count: models.length })}</p>
        )}
      </Field>
  ...
  if (status === 'error') {
    return (
      <p className="flex items-center gap-1 text-[12px] text-error mt-2">
        <XCircle size={13} /> {t('onboarding.connectionFailedConfig')}
      </p>
    )
  }
  return null
